...
function SingUpForm({ handler }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null)
    let res = await handler(name, email, password)
    if (res === 'Firebase: Error (auth/invalid-email).')
      setError('invalid email')
    else if (res === 'Firebase: Password should be at least 6 characters (auth/weak-password).')
  ...
  return (
    <>
      ...
      <form className='flex flex-col w-[95%] items-start justify-center sm:w-[50%] md:w-[40%] lg:w-[35%]  pb-[5%]' onSubmit={handleSubmit} action="">
        ...
        <input type="text"
          className=' w-full 
       text-black bg-white border-[1px] py-[1%] text-[110%]' name="name" value={name} onChange={(e) => { setName(e.target.value) }} />
        ...
        <input type="password" className=' w-full 
         text-black bg-white border-[1px] py-[1%] text-[110%]' name="password" value={password} onChange={(e) => { setPassword(e.target.value) }} />


        {err}


        <label htmlFor="  " className={` ${loading ? "" : "hidden"} mt-[2%] w-full flex flex-row justify-end`}>
