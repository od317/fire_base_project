...
function MessageInput({handleSendingMessage}) {
  ...
  const handleKeyDown = (e)=>{
        if(e.key === 'Enter' && !e.shiftKey && width >= 768){
          e.preventDefault()
  ...
  return (
    ...
    '>
        <form onSubmit={sendMessage} ref={formRef} className='w-full  text-black flex  items-center justify-center  flex-row'>
                <textarea onKeyDown={handleKeyDown} className='
                max-h-[2rem] bg-white p-[.5%] resize-none hideScroll text-[120%] w-[80%] md:w-[86%]
                ' ref={ref} name="" value={message} onChange={(e)=>{
                  setMessage(e.target.value)
                }} id="" cols="30"></textarea>
                <button onClick={sendMessage} className=' w-[20%] md:w-[14%] flex items-center  justify-center'>
                <svg
